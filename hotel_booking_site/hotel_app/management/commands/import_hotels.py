...
class Command(BaseCommand):
    help = "Delete all old hotels and import new ones from mp_hotels.csv"
            ...
            for row in reader:
                try:
                    hotel = Hotel.objects.create(
                        name=row['Hotel Name'].strip(),
                        city=row['City'].strip(),
                        address=row['Address'].strip(),
                        rating=float(row['Rating'].strip()) if row['Rating'].strip() else None,
                        price=row['Price (INR)'].strip(),
                        rooms_available=int(row['Rooms Available'].strip()) if row['Rooms Available'].strip() else 0,
